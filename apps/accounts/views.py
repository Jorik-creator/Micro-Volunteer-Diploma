...
class CustomLogoutView(LogoutView):
    ...
    def post(self, request, *args, **kwargs):
        # Повідомлення додається ПІСЛЯ super().post(), бо auth_logout() очищає сесію.
        # FallbackStorage зберігає повідомлення в cookie-fallback для нової сесії.
...
class ProfileView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        user = (
            self.object
        )  # DetailView вже викликав get_object() і закешував у self.object
        ...
        if user.is_volunteer:
            ...
            context["accepted_count"] = user.volunteer_responses.filter(
                status="accepted"
            ).count()
            ...
            context["completed_count"] = user.help_requests.filter(
                status="completed"
            ).count()
